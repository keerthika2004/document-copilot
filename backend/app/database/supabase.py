...
def _server_client_options(
    *,
    ...
    """Return standard client options configured for server-side stateless usage."""
...
def _normalize_access_token(access_token: str) -> str:
    """Strip 'Bearer ' prefix if present in the header value."""
    prefix = "Bearer "
...
async def get_service_role_client() -> AsyncClient:
    """Return a shared async client that bypasses RLS — backend-only privileged access."""
...
async def create_user_client(access_token: str) -> AsyncClient:
    """Return a request-scoped async client that enforces RLS for the authenticated user."""
