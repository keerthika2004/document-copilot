...
flowchart LR
    ...
    local_embed[Local Embeddings<br/>BAAI/bge-small-en-v1.5]
    ...
    backend -->|generate grounded answer| groq
    backend -->|stream answer + citations| browser

    corpus --> ingestion
    ingestion -->|create embeddings| local_embed
...
- Groq SDK (`llama-3.3-70b-versatile`) via PydanticAI for generation
- Local `fastembed` / `sentence-transformers` (`BAAI/bge-small-en-v1.5`) for 384-dim vector embeddings
...
- `GROQ_API_KEY`
- `ALLOWED_ORIGINS`
- embedding model name (`BAAI/bge-small-en-v1.5`) and dimensions (`384`)
...
8. Add Markdown ingestion, chunking, embeddings (`fastembed`), and Supabase writes.
9. Add semantic search with `pgvector`.
10. Add Postgres full-text search and Python RRF fusion.
11. Add PydanticAI document agent with typed dependencies and typed answer output via Groq (`llama-3.3-70b-versatile`).
...
- No direct LLM or Groq API calls from the browser.
